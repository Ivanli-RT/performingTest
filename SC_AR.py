...
class circleS_accuracy_repearability():
    ...
    def calculate(self):
        ...
        r.truncate(0)
        r.write('***小圆轨迹准确度和小圆轨迹重复性***\n')
        r.write('小圆轨迹准确度为：%.4fmm，在第%d个平面；\n小圆轨迹重复性为：%.4fmm，在第%d个平面；'
        ...
        fig.suptitle('小圆轨迹准确度', fontsize=16)
        # 准确度最大处    
        X = newXYZ[ATp[0]][:,0]
        Y = newXYZ[ATp[0]][:,1]
